  ...
  return (
    <div className="bg-admin-surface border border-admin-border rounded-sm flex-1 p-6">
      <div className="flex items-center gap-2 mb-8">
        ...
        <h4 className="text-[20px] font-semibold text-admin-gray-400">Consumables Reorder</h4>
      </div>
      <div className="flex flex-col gap-4">
        {items.map((item) => (
          <div key={item.name} className="border-b border-admin-border flex items-center justify-between pb-3">
            <span className="text-[13px] leading-4 text-admin-gray-400">{item.name}</span>
            <div className="flex items-center gap-3">
              <span className="text-[13px] font-mono font-medium text-admin-gray-400">{item.stock}</span>
